...
namespace TinySolutions\cptwoointpro\PluginsSupport\EasyBooking;

// Do not allow directly accessing this file.
use EasyBooking\Pickadate;
...
class EasyBookingInit {
	...
	private function register_frontend_scripts( $product ) {
		...
		wp_register_script(
			...
			[ 'jquery', 'pickadate', 'wceb-datepickers' ],
			'1.0',
			true
		);
	}
	...
	private function get_frontend_parameters( $product ) {
		...
		$ajax_url = str_replace( [ 'http:', 'https:' ], '', $ajax_url ); // Fix to avoid security fails.
		// Product type.
		$product_type = $product->get_type();
		// Days or Nights mode.
		...
		// Booking settings and prices for each product type.
		$booking_settings = [];
		$children         = [];
		$prices           = [];
		switch ( $product_type ) {
			...
			case 'grouped':
				...
				if ( $children ) {
					foreach ( $children as $child_id ) {
						...
						$prices['price'][ $child_id ]         = wc_get_price_to_display( $child, [ 'price' => $child->get_price() ] );
						$prices['regular_price'][ $child_id ] = wc_get_price_to_display( $child, [ 'price' => $child->get_regular_price() ] );
					}
				}
