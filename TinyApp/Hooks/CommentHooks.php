...
class CommentHooks {
	...
	public static function init() {
		...
		// Rating posts.
		add_filter( 'comments_open', [ __CLASS__, 'comments_open' ], 15, 2 );
	...
	public static function comments_open( $open, $post_id ) {
		...
		$is_supported = Fns::is_review_enabled( $post_type );
		if ( $is_supported ) {
	...
	public static function check_comment_rating( $comment_data ) {
		$post_type    = get_post_type( absint( $_POST['comment_post_ID'] ?? 0 ) );
		$is_supported = Fns::is_review_enabled( $post_type );
		if ( ! is_admin() && isset( $_POST['comment_post_ID'], $_POST['rating'], $comment_data['comment_type'] ) && $is_supported && empty( $_POST['rating'] ) && self::is_default_comment_type( $comment_data['comment_type'] ) && wc_review_ratings_enabled() && wc_review_ratings_required() ) { // WPCS: input var ok, CSRF ok.
	...
	public static function add_comment_rating( $comment_id ) {
		...
		if ( isset( $_POST['rating'], $_POST['comment_post_ID'] ) && $is_supported ) { // WPCS: input var ok, CSRF ok.
	...
	public static function update_comment_type( $comment_data ) {
		...
		$is_supported = Fns::is_review_enabled( $post_type );
		if ( ! is_admin() && isset( $_POST['comment_post_ID'], $comment_data['comment_type'] ) && self::is_default_comment_type( $comment_data['comment_type'] ) && $is_supported ) { // WPCS: input var ok, CSRF ok.
